...
# Local imports (assuming this file is in a package 'networks')
...
class SSLModel(nn.Module):
    """
    Self-Supervised Learning Model Wrapper for EAT-Large.
    """
    def __init__(self, device: str):
        super().__init__()
        self.device = device
        
        # 1. Load EAT-Large Pretrained Model
        ...
        # Search for the encoder layers in various potential model structures
        search_paths = [
            ['eat', 'encoder', 'layer'], 
    ...
    def extract_feat(self, x: torch.Tensor) -> torch.Tensor:
        """
        Preprocesses audio and extracts features using the SSL model.
        ...
        # --- 2. Forward Pass through SSL Model ---
        self._hook_features = None
        try:
            _ = self.model(mels)
        except Exception:
            # Ignore potential errors if hook captured data successfully
            pass 
...
class Model(nn.Module):
    """
    EAT-Large AASIST Model Architecture.
    Combines SSL features with Heterogeneous Graph Attention Networks.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 5. Graph Attention Layers (Spectral & Temporal)
